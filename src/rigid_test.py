...
def test( iter_num, gpu_id, vol_size=(160,192,224), nf_enc=[16,32,32,32], nf_dec=[32,32,32,32,32,16,16,3], model_name = "vm2_cc", sample_num = 10):
    ...
    x = np.reshape(np.linspace(0, vol_size[0], sample_num + 1), (1, vol_size[sample_num + 1]))
    y = np.reshape(np.linspace(0, vol_size[1], sample_num + 1), (1, vol_size[sample_num + 1]))
    z = np.reshape(np.linspace(0, vol_size[2], sample_num + 1), (1, vol_size[sample_num + 1]))#index
    print(z.shape)
