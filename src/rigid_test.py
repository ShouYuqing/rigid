...
def test( iter_num, gpu_id, vol_size=(160,192,224), nf_enc=[16,32,32,32], nf_dec=[32,32,32,32,32,16,16,3], model_name = "vm2_cc", sample_num = 10):
    ...
    # Y in formula
    x_flow = np.reshape(np.arange(vol_size[0]), (1, vol_size[0]))
    y_flow = np.reshape(np.arange(vol_size[1]), (1, vol_size[1]))
    z_flow = np.reshape(np.arange(vol_size[2]), (1, vol_size[2]))
    print(z_flow.shape)
