...
def rigid_net(vol_size, enc_nf, dec_nf):
    ...
    tf.reshape(flow1,shape = [1,flow1.get_shape()[1].value,flow1.get_shape()[2].value,
                              flow1.get_shape()[3].value,1])
    flow2 = flow[0,:,:,:,1]
    tf.reshape(flow2, shape=[1, flow2.get_shape()[1].value, flow2.get_shape()[2].value,
                             flow2.get_shape()[0].value, 1])
    flow3 = flow[0,:,:,:,2]
    tf.reshape(flow1, shape=[1, flow3.get_shape()[1].value, flow3.get_shape()[2].value,
                             flow3.get_shape()[3].value, 1])
